...
namespace FakeXrmEasy
{
    public partial class XrmFakedContext : IXrmContext
    {
        ...
        protected IPluginExecutionContext GetFakedPluginContext(XrmFakedPluginExecutionContext ctx)
        {
            ...
            A.CallTo(() => context.BusinessUnitId).ReturnsLazily(() => ctx.BusinessUnitId == Guid.Empty ? Guid.NewGuid() : ctx.BusinessUnitId);
            A.CallTo(() => context.CorrelationId).ReturnsLazily(() => ctx.CorrelationId == Guid.Empty ? Guid.NewGuid() : ctx.CorrelationId);
            ...
            A.CallTo(() => context.IsOfflinePlayback).ReturnsLazily(() => ctx.IsOfflinePlayback);
            A.CallTo(() => context.IsolationMode).ReturnsLazily(() => ctx.IsolationMode == 0 ? 1 : 2);
            A.CallTo(() => context.MessageName).ReturnsLazily(() => ctx.MessageName);
            A.CallTo(() => context.Mode).ReturnsLazily(() => ctx.Mode);
            A.CallTo(() => context.OperationCreatedOn).ReturnsLazily(() => ctx.OperationCreatedOn == DateTime.MinValue ? DateTime.UtcNow : ctx.OperationCreatedOn);
            A.CallTo(() => context.OperationId).ReturnsLazily(() => ctx.OperationId == Guid.Empty ? Guid.NewGuid() : ctx.OperationId);
            A.CallTo(() => context.OrganizationId).ReturnsLazily(() => ctx.OrganizationId == Guid.Empty ? Guid.NewGuid() : ctx.OrganizationId);
            A.CallTo(() => context.OrganizationName).ReturnsLazily(() => ctx.OrganizationName ?? "Default organization");
            ...
            A.CallTo(() => context.RequestId).ReturnsLazily(() => ctx.RequestId == Guid.Empty ? Guid.NewGuid() : ctx.RequestId);
